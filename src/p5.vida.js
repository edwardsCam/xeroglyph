...
*/
p5.prototype.VidaBlob = VidaBlob

function VidaBlob(_sketch) {
  /*
...
p5.prototype.Vida = function (_sketch) {
  ...
  */
  ...
  /*
    ...
    image being processed.
  */
  this.resizeBlobMapArray(this.thresholdImage.width, this.thresholdImage.height)
}
...
p5.prototype.Vida.prototype.resetBlobMapArray = function () {
  for (let y = 0; y < this.thresholdImage.height; y++)
    for (let x = 0; x < this.thresholdImage.width; x++)
...
p5.prototype.Vida.prototype.setActiveZonesNormFillThreshold = function (
  ...
  for (let i = 0; i < this.activeZones.length; i++)
    this.activeZones[i].normFillThreshold = this.__activeZonesNormFillThreshold
}
...
p5.prototype.Vida.prototype.addActiveZone = function (
  ...
  */
  ...
  if (
    ...
    _onChangeCallbackFunction = function (_activeZone) {}
  this.activeZones[this.activeZones.length] = new this.__sketch.VidaActiveZone(
    _id,
    _normX,
    _normY,
    _normW,
...
*/
p5.prototype.Vida.prototype.drawBlobs = function (_x, _y, props) {
  /*
    Thanks of this condition we can use this function with shorter or longer
    list of paramewters.
  */
  const _w = this.thresholdImage.width
  const _h = this.thresholdImage.height

  // some reusable variables we need
  let temp_rect_x, temp_rect_y, temp_rect_w, temp_rect_h
  // temp_mass_center_x,
  // temp_mass_center_y
  ...
  for (let i = 0; i < this.__blobs[this.__currentBlobsLocation].length; i++) {
    ...
    if (props.drawBoundingBox) {
      ...
      temp_rect_y = Math.floor(blob.normRectY * _h)
      temp_rect_w = Math.floor(blob.normRectW * _w)
      temp_rect_h = Math.floor(blob.normRectH * _h)
      // temp_mass_center_x = Math.floor(blob.normMassCenterX * _w)
      // temp_mass_center_y = Math.floor(blob.normMassCenterY * _h)
      ...
      // this.__sketch.fill(0, 255, 0)
      // this.__sketch.ellipseMode(this.__sketch.CENTER)
      // this.__sketch.ellipse(temp_mass_center_x, temp_mass_center_y, 3, 3)
      // print id
      // this.__sketch.noStroke()

      if (props.showBlobInfo) {
        this.__sketch.fill(255, 255, 0)
        this.__sketch.text(ampedMass.toFixed(3), temp_rect_x, temp_rect_y - 1)
      }
    }

    // draw approximated polygon (if available)
    const strokeWeight = Math.floor(
      interpolate(
        [this.normMinBlobMass, this.normMaxBlobMass],
        [1, 5],
        ampedMass
      )
    )
    this.__sketch.strokeWeight(strokeWeight)
    this.__sketch.stroke('rgba(255, 0, 0, 0.9)')
    ...
    this.__sketch.fill(`rgba(${colorVal}, 255, ${colorVal}, 0.9)`)

    this.__sketch.beginShape()
    blob.approximatedPolygon.forEach((polygon) => {
      this.__sketch.vertex(polygon.normX * _w, polygon.normY * _h)
    })
...
p5.prototype.Vida.prototype.drawActiveZones = function (_x, _y, _w, _h) {
  ...
  */
  ...
  // some reusable variables we need
  let temp_coord_x, temp_coord_y, temp_coord_w, temp_coord_h
  ...
  for (let i = 0; i < this.activeZones.length; i++) {
    ...
    // print id
    this.__sketch.noStroke()
    if (this.activeZones[i].isEnabledFlag) this.__sketch.fill(255, 0, 0)
    else this.__sketch.fill(0, 0, 255)
    this.__sketch.text(this.activeZones[i].id, temp_coord_x, temp_coord_y - 1)
  }
...
p5.prototype.Vida.prototype.updateActiveZones = function () {
  ...
  for (let i = 0; i < this.activeZones.length; i++) {
    ...
    for (let y = temp_coord_start_y; y <= temp_coord_end_y; y++) {
      for (let x = temp_coord_start_x; x <= temp_coord_end_x; x++) {
    ...
    */
    if (
      this.activeZones[i].normFillFactor > this.activeZones[i].normFillThreshold
    )
      temp_isMovementDetectedFlag = true
    else temp_isMovementDetectedFlag = false
  ...
  */
  ...
  if (this.progressiveBackgroundFlag) {
    ...
    for (let i = 0; i < this.backgroundImage.pixels.length; i += 4) {
      ...
      this.differenceImage.pixels[i] = Math.abs(
        this.backgroundImage.pixels[i] - this.currentImage.pixels[i]
      )
      this.differenceImage.pixels[i + 1] = Math.abs(
        this.backgroundImage.pixels[i + 1] - this.currentImage.pixels[i + 1]
      )
      this.differenceImage.pixels[i + 2] = Math.abs(
...
p5.prototype.Vida.prototype.trackBlobs = function () {
  ...
  */
  if (this.__blobs[this.__previousBlobsLocation].length < 1) {
    ...
    */
    for (let i = 0; i < this.__blobs[this.__previousBlobsLocation].length; i++)
      if (this.__blobs[this.__previousBlobsLocation][i].id < 0)
        this.__blobs[this.__previousBlobsLocation][i].id = this.findFirstFreeId(
  ...
  */
  for (let i = 0; i < this.numberOfDetectedBlobs; i++) {
    for (let j = 0; j < this.numberOfDetectedBlobs; j++) {
  ...
  for (let i = 0; i < this.numberOfDetectedBlobs; i++) {
    for (let j = 0; j < this.numberOfDetectedBlobs; j++) {
      ...
      if (this.__blobs[this.__currentBlobsLocation][i].id < 0) {
        this.__blobs[this.__currentBlobsLocation][i].id = this.findFirstFreeId(
          this.__currentBlobsLocation
        )
        continue
      }
      ...
      if (this.__blobs[this.__currentBlobsLocation][j].id < 0) {
        this.__blobs[this.__currentBlobsLocation][j].id = this.findFirstFreeId(
          this.__currentBlobsLocation
        )
        continue
      }
...
p5.prototype.Vida.prototype.approximateBlobPolygons = function () {
  ...
  */
  ...
  for (let i = 0; i < this.numberOfDetectedBlobs; i++) {
    ...
    */
    for (let j = 0; j < this.pointsPerApproximatedBlobPolygon; j++) {
      /*
...
p5.prototype.Vida.prototype.rejectInnerBlobs = function () {
  for (let i = this.numberOfDetectedBlobs - 1; i >= 0; i--) {
    for (let j = this.numberOfDetectedBlobs - 1; j >= 0; j--) {
  ...
  for (let i = 0; i < this.numberOfDetectedBlobs; i++) {
    ...
    */
    if (
      ...
      continue // blob is too small or too heavy, skip it
    /*
...
p5.prototype.Vida.prototype.findBlobs = function () {
  ...
  */
  let temp_previousNumberOfIdentifiers = -1
  let temp_currentNumberOfIdentifiers = 0
  /*
  ...
  */
  while (temp_previousNumberOfIdentifiers !== temp_currentNumberOfIdentifiers) {
    ...
    */
    ...
    temp_currentNumberOfIdentifiers = this.findBlobs_countUnorderedIdentifiers()
  }
  /*
...
p5.prototype.Vida.prototype.findBlobs_createTemporaryIndices = function () {
  let temp_pixel_position // index of the pixel we want to test
  let temp_blob_identifier = 1 // as the name suggests
  /*
  ...
  for (let temp_y = 1; temp_y < temp_hmax; temp_y++) {
    for (let temp_x = 1; temp_x < temp_wmax; temp_x++) {
      ...
      */
      ...
      else temp_blob_identifier += 1 // increment value when pixel is empty
    }
  }
}
...
p5.prototype.Vida.prototype.findBlobs_mergerIterationA = function () {
  /*
    ...
    the north, clockwise.
  */
  let temp_nn, temp_ne, temp_ee, temp_se, temp_ss, temp_sw, temp_ww, temp_nw
  /*
    To prevent testing for each cell, whether it lies on the edge of the array
    (which would require additional conditional instructions and slow down the
    code significantly) the function will only operate on the "internal" cells
...
p5.prototype.Vida.prototype.findBlobs_mergerIterationB = function () {
  ...
  */
  ...
  /*
    ...
    corner.
  */
  for (let temp_x = temp_wmax; temp_x > 0; temp_x--) {
    for (let temp_y = temp_hmax; temp_y > 0; temp_y--) {
      // empty cell (no blob), move to the next one
      if (this.__blobMapArray[temp_x][temp_y] === 0) continue
      // collect statuses of neighboring cells
      temp_nn = this.__blobMapArray[temp_x][temp_y - 1]
  ...
  */
  for (let temp_y = 1; temp_y < temp_hmax; temp_y++) {
    for (let temp_x = 1; temp_x < temp_wmax; temp_x++) {
      ...
      for (let i = 0; i < temp_redirections_array.length; i++) {
        /*
  ...
  */
  for (let temp_y = 1; temp_y < temp_hmax; temp_y++) {
    for (let temp_x = 1; temp_x < temp_wmax; temp_x++) {
      ...
      for (let i = 0; i < temp_identifiers_array.length; i++) {
        /*
          If current identifier is already in the list we can set optimal
          identifier, switch off the flag and escape from the loop.
        */
        if (temp_identifiers_array[i] === this.__blobMapArray[temp_x][temp_y]) {
          temp_b = false
          break
        }
      }
...
export default p5.prototype.Vida
